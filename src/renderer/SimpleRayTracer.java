...
/**
 * Simple recursive ray tracer supporting local lighting, shadows,
 * reflection, and transparency.
 * <p>
 * All intermediate values are kept in stack-local records so the class
 * is fully thread-safe with no shared mutable state.
 */
class SimpleRayTracer extends RayTracerBase {
    ...
    private Color calcColor(Intersectable.Intersection intersection, Ray ray) {
        ...
        return ctx == null ? ambient
                           : ambient.add(calcColor(intersection, ctx, MAX_CALC_COLOR_LEVEL, INITIAL_K));
    }

    private Color calcColor(Intersectable.Intersection intersection, IntersectionCtx ctx,
                            int level, Double3 k) {
        Color color = calcLocalEffects(intersection, ctx, k);
        return level == 1 ? color : color.add(calcGlobalEffects(intersection, ctx, level, k));
    }

    // ── global effects (reflection + transparency) ────────────────────────────

    private Color calcGlobalEffects(Intersectable.Intersection intersection, IntersectionCtx ctx,
                                    int level, Double3 k) {
    ...
    private Color calcLocalEffects(Intersectable.Intersection intersection, IntersectionCtx ctx,
                                   Double3 k) {
        ...
        for (LightSource ls : _scene.lights) {
            ...
            if (ktr.product(k).isGreaterThan(MIN_CALC_COLOR_K)) {
                color = color.add(
                        lCtx.intensity().scale(ktr).scale(
                                calcDiffusive(intersection, lCtx)
                                        .add(calcSpecular(intersection, ctx, lCtx))));
            }
        }
        return color;
    }
    ...
    private Double3 calcDiffusive(Intersectable.Intersection intersection, LightCtx lCtx) {
        return intersection.material.kD.scale(abs(lCtx.nl()));
    }

    private Double3 calcSpecular(Intersectable.Intersection intersection,
                                 IntersectionCtx ctx, LightCtx lCtx) {
        Material m  = intersection.material;
    ...
    private Double3 transparency(Intersectable.Intersection intersection,
        ...
        double  lightDist = lCtx.source().getDistance(intersection.point);
